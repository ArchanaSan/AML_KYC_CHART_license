...
if (err) throw err;
var sql = "select month,IFNULL(FORMAT(a,0), 0) amount ,order1 from month LEFT JOIN (SELECT SUM(amount*rate) a,DATE_FORMAT(tx_timestamp, \"%M %Y\") b, tx_timestamp from exchange_rate x1,transactions_transaction c where c.fromaddress = '" + id + "' AND x1.from_token = C.token AND DATE_FORMAT(C.tx_timestamp, \"%Y-%m-%d\") = x1.created_date  group by b order by tx_timestamp) Y ON DATE_FORMAT(month.tx_timestamp, \"%M %Y\") = Y.B order by order1 desc";
...
var sql = "select month,IFNULL(FORMAT(a,0), 0) amount,order1 from month LEFT JOIN (SELECT SUM(amount*rate) a,DATE_FORMAT(tx_timestamp, \"%M %Y\") b, tx_timestamp from exchange_rate x1,transactions_transaction c where c.toaddress = '" + id + "'  AND x1.from_token = C.token AND DATE_FORMAT(C.tx_timestamp, \"%Y-%m-%d\") = x1.created_date group by b order by tx_timestamp) Y ON DATE_FORMAT(month.tx_timestamp, \"%M %Y\") = Y.B order by order1 desc";
...
var sql = "select month,IFNULL(FORMAT(a,0), 0) amount,order1 from month LEFT JOIN (SELECT SUM(amount*rate) a,DATE_FORMAT(tx_timestamp, \"%M %Y\") b, tx_timestamp from exchange_rate x1,transactions_transaction c where (c.toaddress = '" + id + "' or c.fromaddress = '" + id + "')  AND x1.from_token = C.token AND DATE_FORMAT(C.tx_timestamp, \"%Y-%m-%d\") = x1.created_date  group by b order by tx_timestamp) Y ON DATE_FORMAT(month.tx_timestamp, \"%M %Y\") = Y.B order by order1 desc";
...
var sql = "select *, a.amount*rate amount_usd, a.amount amount_token from exchange_rate d,transactions_transaction a,authentication_kycuserdetail c, (SELECT DISTINCT * FROM rule_2_hour ) b where a.id=b.trans_id and a.user_id = c.user_id and c.wallet_id ='" + id + "' and d.from_token = a.token AND DATE_FORMAT(a.tx_timestamp, \"%Y-%m-%d\") = d.created_date and c.user_id = b.user_id";
db.query(sql, function (err, rule_2_hour, fields) { if (err) throw err;
...
res.render('singleuser',{ title : 'User Details', user_details : user_details, basic_rules : basic_rules, advanced_rules : advanced_rules,
						...
						rule_2_hour : rule_2_hour,message : 'KYC SECTION' } )
})})})})})})})})})})})})})})})})})})
...
function getSQL(ruleNo, transactionvalue) {
    if (ruleNo == 'VA-OSS') { var sql = "SELECT DISTINCT * FROM transactions_transaction a,exchange_rate b WHERE amount*rate > " + transactionvalue + " AND STATUS = '1' AND a.token = b.from_token AND DATE_FORMAT(a.tx_timestamp, \"%Y-%m-%d\") = b.created_date order by fromAddress";  }
  else if (ruleNo == 'VA-OSD') { var sql = "SELECT *  FROM transactions_transaction e,( SELECT SUM(amount*rate) a,DATE_FORMAT(tx_timestamp, \"%M %d %Y\") b,toAddress c  FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date  GROUP BY b,c HAVING a > " + transactionvalue + ") f where e.toaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %d %Y') = f.b and e.status = '1'";}
 else if (ruleNo == 'VA-OSM') { var sql = "SELECT *  FROM transactions_transaction e,( SELECT SUM(amount*rate) a,DATE_FORMAT(tx_timestamp, \"%M %Y\") b,toAddress c  FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY b,c HAVING a > " + transactionvalue + ") f where e.toaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %Y') = f.b and e.status = '1'";}
  else if (ruleNo == 'VA-OMD') { var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2 FROM transactions_transaction e, (SELECT SUM(amount*rate) a,fromaddress b, DATE_FORMAT(tx_timestamp, \"%Y-%m-%d\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY b,d HAVING a > " + transactionvalue + ") f where e.fromaddress = f.b and DATE_FORMAT(e.tx_timestamp, '%Y-%m-%d') = f.d and e.status = '1') a group by e1,e2 having a1 <2";}
 else if (ruleNo == 'VA-OMM') { var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2 FROM transactions_transaction e,(select sum(a) a, b, DATE_FORMAT(d, \"%M %Y\") d from (SELECT SUM(amount*rate) a,fromaddress b, DATE_FORMAT(tx_timestamp, \"%Y-%m-%d\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY b,d) z1 group by DATE_FORMAT(d, \"%M %Y\"),b having a > 50000) f where e.fromaddress = f.b and DATE_FORMAT(e.tx_timestamp, '%M %Y') = f.d and e.status = '1') a group by e1,e2 having a1 <2";}
  else if (ruleNo == 'VA-BSS') { var sql = "SELECT * FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date and amount*rate > " + transactionvalue + " and status = '1'";  }
  else if (ruleNo == 'VA-BSD') { var sql = "select *  from transactions_transaction e,(SELECT SUM(amount*rate) a,fromaddress b, toaddress c,DATE_FORMAT(tx_timestamp, \"%M %d %Y\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY b,c,d HAVING a > " + transactionvalue + ") f where e.fromaddress = f.b and e.toaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %d %Y') = f.d and e.status = '1'";}
  else if (ruleNo == 'VA-BSM') { var sql = "select *  from transactions_transaction e,(SELECT SUM(amount*rate) a,fromaddress b, toaddress c,DATE_FORMAT(tx_timestamp, \"%M %Y\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY b,c,d HAVING a > " + transactionvalue + ") f where e.fromaddress = f.b and e.toaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %Y') = f.d and e.status = '1'";}
  else if (ruleNo == 'VA-BMD') { var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2  from transactions_transaction e, (SELECT SUM(amount*rate) a, toaddress c,DATE_FORMAT(tx_timestamp, \"%M %d %Y\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY c,d HAVING a > " + transactionvalue + ") f   where e.toaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %d %Y') = f.d and e.status = '1') a group by e1,e2 having a1 <2";}
  else if (ruleNo == 'VA-BMM') { var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2  from transactions_transaction e,(SELECT SUM(amount*rate) a, toaddress c,DATE_FORMAT(tx_timestamp, \"%M %Y\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY c,d HAVING a > " + transactionvalue + ") f where e.toaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %Y') = f.d and e.status = '1') a group by e1,e2 having a1 <2";}
 ...
 else if (ruleNo == 'VO-OMD') { var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2  from transactions_transaction e,(SELECT COUNT(id) a, fromaddress c,DATE_FORMAT(tx_timestamp, \"%M %d %Y\") d FROM transactions_transaction GROUP BY c,d HAVING a > " + transactionvalue + ") f where e.fromaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %d %Y') = f.d) a group by e1,e2 having a1 <2";}
  else if (ruleNo == 'VO-OMM') { var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2  from transactions_transaction e,(SELECT COUNT(id) a, fromaddress c,DATE_FORMAT(tx_timestamp, \"%M %Y\") d FROM transactions_transaction GROUP BY c,d HAVING a > " + transactionvalue + ") f where e.fromaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %Y') = f.d) a group by e1,e2 having a1 <2";}
  ...
  else if (ruleNo == 'VO-BMD') { var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2  FROM transactions_transaction e, (SELECT COUNT(id) a, toaddress c, DATE_FORMAT(tx_timestamp, \"%M %d %Y\") d FROM transactions_transaction GROUP BY c,d HAVING a > " + transactionvalue + ") f where e.toaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %d %Y') = f.d) a group by e1,e2 having a1 <2";}
  else if (ruleNo == 'VO-BMM') { var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2  FROM transactions_transaction e, (SELECT COUNT(id) a, toaddress c, DATE_FORMAT(tx_timestamp, \"%M %Y\") d FROM transactions_transaction GROUP BY c,d HAVING a > " + transactionvalue + ") f where e.toaddress = f.c and DATE_FORMAT(e.tx_timestamp, '%M %Y') = f.d) a group by e1,e2 having a1 <2";}
 ...
 app.get('/gender', function (req, res) {
   ...
   db.query(sql1, function (err, result, fields) {
     ...
     db.query(sql1, function (err, result, fields) {
       ...
       // Average Volume of Transaction Yesterday QUERY SUBMISSION

       app.get('/aveVolumeTransDaily1', function (req, res) {
       res.render('aveVolumeTransDaily',
         { title : 'aveVolumeTransDaily Details', message : 'This shows aveVolumeTransDaily' }
           )
         })

       // Average Volume of Transaction Yesterday QUERY RESULT

       app.get('/aveVolumeTransDaily2', function (req, res) {
       //var id = req.query.id;
       var sql1 = "SELECT CONCAT(5*floor(TransactionCount/5), '-', 5*floor(TransactionCount/5) + 5) as `TransactionCountRenge`,  COUNT(*) as count FROM (SELECT user_id, count(*) as TransactionCount  FROM wallet.transactions_transaction WHERE DATE(tx_timestamp) = SUBDATE(CURRENT_DATE,1) group by user_id)as t group by `TransactionCountRenge` ORDER BY TransactionCountRenge ASC" ;
       db.query(sql1, function (err, result, fields) {
       if (err) throw err;
       res.render('aveVolumeTransDaily',{ title : 'User Details', users : result} )});})


       // Average Volume of Transaction Yesterday QUERY SUBMISSION

       app.get('/timeOfTrans1', function (req, res) {
       res.render('timeOfTrans',
         { title : 'timeOfTrans Details', message : 'This shows timeOfTrans' }
           )
         })

       // Average Volume of Transaction Yesterday QUERY RESULT

       app.get('/timeOfTrans2', function (req, res) {
       //var id = req.query.id;
       var sql1 = "SELECT CONCAT(timerange, ':00-', timerange,':59') as `TransactionCountRenge`,  COUNT(*) as count FROM (SELECT user_id, count(tx_timestamp) as TransactionCount, EXTRACT(HOUR FROM tx_timestamp) as timerange FROM transactions_transaction  WHERE MONTH(tx_timestamp) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH) group by user_id) as t group by `TransactionCountRenge` ORDER BY TransactionCountRenge DESC" ;
       db.query(sql1, function (err, result, fields) {
       if (err) throw err;
       res.render('timeOfTrans',{ title : 'User Details', users : result} )});})
