...
function rule_VAOSS() {
	...
	var sql = "SELECT distinct * FROM transactions_transaction a,exchange_rate b WHERE amount*rate > " + transactionvalue + " AND STATUS = '1' AND a.token = b.from_token AND DATE_FORMAT(a.tx_timestamp, \"%Y-%m-%d\") = b.created_date";
	db.query(sql, function (err, result, fields) { if (err) throw err;
	if (result) 
		   {  
			  var n = result.length;			  
	...
	if (result) 
		   {  
			  ...
			  for (i = 0; i < result.length; i++) {							
                        var trans_id = result[i].trans_id;
                        var user_id = result[i].user_id;						
						var sql1 = "UPDATE rules_broken_customer_detailed SET rules = CONCAT(rules, '|VA-OSB|') WHERE user_id='" + user_id + "' and trans_id='" + trans_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});
	...
	if (result) 
		   {  
			  ...
			  for (i = 0; i < result.length; i++) {	
						...
						var user_id = result[i].user_id;						
						var trans_id = result[i].trans_id;						 
						var sql1 = "UPDATE rules_broken_customer_detailed SET rules = CONCAT(rules, '|VA-OMB|') WHERE user_id='" + user_id + "' and trans_id='" + trans_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});
...
function rule_VABSB() {
	var user_id1 = '';
	transactionvalue = 5000;
	var sql = "SELECT DISTINCT * FROM rule_2_hour WHERE RULE = 'VA-BSB'";
	db.query(sql, function (err, result, fields) { if (err) throw err;
	if (result) 
		   {  
			  var n = result.length;			  
			  for (i = 0; i < result.length; i++) {							
                        var trans_id = result[i].trans_id;
                        var user_id = result[i].user_id;						
						var sql1 = "UPDATE rules_broken_customer_detailed SET rules = CONCAT(rules, '|VA-BSB|') WHERE user_id='" + user_id + "' and trans_id='" + trans_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});
...
function rule_VOOMD() {
	var user_id1 = '';
	transactionvalue = 20;
	var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2 FROM transactions_transaction e,(SELECT SUM(amount*rate) a,fromaddress b, DATE_FORMAT(tx_timestamp, \"%Y-%m-%d\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY b,d HAVING a > 20) f where e.fromaddress = f.b and DATE_FORMAT(e.tx_timestamp, '%Y-%m-%d') = f.d and e.status = '1') a group by e1,e2 having a1 <2";
	db.query(sql, function (err, result, fields) { if (err) throw err;
	if (result) 
		   {  
			  var n = result.length;			  
...
function rule_VOOMM() {
	var user_id1 = '';
	transactionvalue = 100;
	var sql = "SELECT  *,COUNT(*) a1 FROM (SELECT *, e.fromaddress e1, e.toaddress e2 FROM transactions_transaction e,(SELECT SUM(amount*rate) a,fromaddress b, DATE_FORMAT(tx_timestamp, \"%M %Y\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY b,d HAVING a > 100) f where e.fromaddress = f.b and DATE_FORMAT(e.tx_timestamp, \"%M %Y\") = f.d and e.status = '1') a group by e1,e2 having a1 <2";
	db.query(sql, function (err, result, fields) { if (err) throw err;
	if (result) 
		   {  
			  var n = result.length;			  
	...
	if (result) 
		   {  
			  ...
			  for (i = 0; i < result.length; i++) {					
						var user_id = result[i].user_id;						
						var trans_id = result[i].id;						 
						var sql1 = "UPDATE rules_broken_customer_detailed SET rules = CONCAT(rules, '|VO-BMM|') WHERE user_id='" + user_id + "' and trans_id='" + trans_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});
	...
	if (result) 
		   {  
			  ...
			  for (i = 0; i < result.length; i++) {
                        var user_id = result[i].user_id;						
						var trans_id = result[i].id;						 
						var sql1 = "UPDATE rules_broken_customer_detailed SET rules = CONCAT(rules, '|BL-OSS|') WHERE user_id='" + user_id + "' and trans_id='" + trans_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});   				  
	...
	if (result) 
		   {  
			  ...
			  for (i = 0; i < result.length; i++) {
                        var user_id = result[i].user_id;						
						var trans_id = result[i].id;						 
						var sql1 = "UPDATE rules_broken_customer_detailed SET rules = CONCAT(rules, '|BL-BSS|') WHERE user_id='" + user_id + "' and trans_id='" + trans_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});   				  
						
					}
	   
		   }
		   });
		   console.log('Finished rule_BLBSS');
}
